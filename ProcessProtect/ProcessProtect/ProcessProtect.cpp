...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING) {
	...
	do {
		...
		if (!NT_SUCCESS(status)) {
			KdPrint((DRIVER_PREFIX "Failed to crate device object (status=%08X)\n", status));
			break;
		}

		status = IoCreateSymbolicLink(&symName, &deviceName);
		if (!NT_SUCCESS(status)) {
			KdPrint((DRIVER_PREFIX "Failed to crate symbolic link (status=%08X)\n", status));
			break;
		}
	} while (false);
	...
	DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = ProcessProtectCreateClose;
